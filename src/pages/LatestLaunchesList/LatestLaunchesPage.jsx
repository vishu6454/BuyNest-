...
import LatestLaunchesData from "../../data/latestLaunchesData";
...
const LatestLaunchesPage = () => {
  ...
  useEffect(() => {
    const foundProduct = LatestLaunchesData.find(
  ...
  const handleCheckoutProcess = () => {
    ...
    if (!currentUser) {
      navigate("/Sign", {
        state: { highlightLogin: true, from: `/latest-launches/product/${id}` },
      });
      return;
    }
    handleAddToCart();
