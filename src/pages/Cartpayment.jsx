...
const CartPayment = () => {
  ...
  const { cartItems, totalAmount, selectedAddress, discount, promoCode, subtotal, totalItems } = state || {};
  ...
  return (
    <motion.div className="max-w-6xl mx-auto px-4 py-12 grid grid-cols-1 lg:grid-cols-2 gap-10" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
      <div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-between items-center pt-4">
            <Link to="/cart/checkout" className="text-blue-600 hover:underline">← Back</Link>
            <button
              type="submit"
      ...
      <motion.div className="bg-white p-6 rounded-xl shadow space-y-4 h-fit" initial={{ y: 30, opacity: 0 }} animate={{ y: 0, opacity: 1 }}>
        ...
        {promoCode && <p className="text-sm text-green-600 pt-2">Promo Applied: {promoCode}</p>}
