...
import MobileBrandData from "../../data/mobileBrandData";
...
const MobilePage = () => {
  ...
  useEffect(() => {
    const foundProduct = MobileBrandData.find(
  ...
  const handleCheckoutProcess = () => {
    if (!selectedSize) {
      toast.error("Please select a storage size before adding to cart.");
      return;
    }
    if (!currentUser) {
      navigate("/Sign", {
        state: { highlightLogin: true, from: `/mobiles/product/${id}` },
      });
      return;
    }
    handleAddToCart();
  ...
  const handleBuyNow = () => {
    if (!selectedSize) {
      toast.error("Please select a storage size before buying.");
      return;
    }
    if (!currentUser) {
      navigate("/Sign", {
      ...
      <div className="lg:grid lg:grid-cols-2 lg:gap-8">
        ...
        <div className="mt-8 lg:mt-0">
          ...
          <div className="mt-4 space-y-1 text-sm text-gray-700">
            ...
            <p>
              <span className="font-medium">Model:</span> {product.variant}
            </p>
            <p>
