...
import ProductCard from '../components/ShopProductCards/ProductCard';
import FilterSidebar from '../components/FilterSidebar';
import MenproductsData from '../data/MenproductsData';
import '../style/Navbar.css';

const ProductListing = () => {
  const [products] = useState(MenproductsData);
  ...
  return (
    ...
    >
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <main className="w-full md:w-3/4">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-4">
            <h1 className="text-2xl font-bold text-gray-900">Men's Fashion</h1>
      ...
      {/* Bottom mobile filter/sort toggle */}
<div className="fixed bottom-4 left-1/2 -translate-x-1/2 w-[90%] md:hidden">
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-end">
      <div className="bg-white w-full max-h-[90vh] overflow-y-auto rounded-t-xl p-4">
        <div className="flex justify-between items-center mb-4">
          ...
          <button onClick={() => setShowFilters(false)} className="text-gray-600 text-sm">
            Close
          </button>
        </div>
        <FilterSidebar
  ...
  {/* Mobile Sort Sidebar Modal */}
  {showSort && (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-end">
      <div className="bg-white w-full max-h-[50vh] overflow-y-auto rounded-t-xl p-4">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-semibold">Sort By</h2>
          <button onClick={() => setShowSort(false)} className="text-gray-600 text-sm">
            Close
          </button>
        </div>
        <select
