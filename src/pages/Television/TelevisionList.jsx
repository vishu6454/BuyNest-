...
import TelevisionCard from "../../components/ShopProductCards/TelevisionCard";
import FilterSidebar from "../../components/FilterSidebar";
import TelevisionsData from "../../data/televisionsData";
import "../../style/Navbar.css";

function TelevisionList() {
  const [products] = useState(TelevisionsData);
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (filters.size.length > 0) {
      result = result.filter((product) =>
        product.sizes.some((size) => filters.size.includes(size.value))
      );
    }
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <main className="w-full md:w-3/4">
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-4 gap-4">
            <h1 className="text-2xl font-bold text-gray-900">Televisions</h1>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 gap-4">
            {filteredProducts.length > 0 ? (
              filteredProducts.map((product) => (
                ...
                >
                  <TelevisionCard product={product} />
                </motion.div>
              ))
            ) : (
